# Another baseline, which is a seq2seq model with gradient reversal layer.
...
class MT5ForConditionalGenerationWithGradReversal(MT5ForConditionalGeneration):
    """
    MT5ForConditionalGeneration with gradient reversal layer.
    """
    def __init__(self, config):
        super().__init__(config)
        self.gradient_reverse_lambda = 0
        self.gradient_reverse_loss = torch.nn.CrossEntropyLoss(reduction='mean')
        self.domain_predictor_ff = torch.nn.Linear(in_features=config.d_model, out_features=5) # For korean, telugu, english, chinese, vietnamese
...
def _train(opt):
    ...
    for epoch in range(opt.epochs):
        ...
        if is_local_main_process and epoch >=5:
            # We only save model with epoch 5, since model with earlier epoch produced invalid sql, resulting in massive evaluation time
